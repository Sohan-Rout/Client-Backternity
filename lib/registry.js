...
GITHUB_REDIRECT_URI=http://localhost:3000/api/oauth/github/callback`,
  ...
  },
  {
    ...
    method: "GET",
    description: "Handles GitHub's callback, issues JWT, and returns the user profile.",
    sampleRequest: "GET /api/oauth/github/callback?code=github_auth_code",
    sampleResponse: {
      success: true,
...
FRONTEND_URL=http://localhost:3000`,
  ...
  },
  {
    ...
    method: "GET",
    description: "Handles Google's callback, issues JWT, and redirects the user to the frontend with the token.",
...
RATE_LIMIT_MAX_REQUESTS=100`,
      ...
      frontendUsage: {
        overview: "The rate limiter protects your API endpoints from abuse. When rate limits are exceeded, specific HTTP responses are returned.",
        endpoints: [
          {
            route: "/api/test-endpoint",
            method: "GET",
            description: "Test any protected endpoint to see rate limiting in action.",
            sampleRequest: null,
            sampleResponse: {
              success: true,
        ...
        endpoints: [
          {
            route: "/api/log-event",
            method: "POST",
            description: "Send a custom log event from frontend to backend logger.",
            sampleRequest: {
              level: "info",
...
},
{
  route: "/error",
  method: "GET",
  description: "Simulates a server error and logs it through Winston.",
  sampleResponse: {
    success: false,
        ...
        endpoints: [
          {
            route: "/api/jobs/add",
            method: "POST",
            description: "Adds a new job to the queue with optional delay or retries.",
            sampleRequest: {
              type: "send-email",
              data: {
                to: "user@example.com",
        ...
        endpoints: [
          ...
          },
          {
            ...
            sampleResponse: {
              ...
              details: "Only JPEG, PNG, and PDF files are allowed. Maximum size: 5MB."
            }
          }
        ],
        ...
        endpoints: [
          {
            ...
            sampleRequest: {
              question: "What are the benefits of using Node.js for backend development?"
            },
            sampleResponse: {
              success: true,
              ...
              data: {
                question: "What are the benefits of using Node.js for backend development?",
                answer: "Node.js offers several key benefits for backend development: 1) Non-blocking I/O for high performance, 2) JavaScript ecosystem and npm packages, 3) Fast V8 engine, 4) Great for real-time applications, 5) Large community support, and 6) Cross-platform compatibility.",
                model: "gpt-4o-mini",
                tokens: {
                  prompt: 15,
                  completion: 67,
        ...
        endpoints: [
          {
            ...
            sampleResponse: {
              ...
              data: {
                question: "Explain the concept of microservices architecture and its advantages",
                answer: "Microservices architecture is a design approach where applications are built as a collection of small, independent services. Key advantages include: 1) Independent deployment and scaling, 2) Technology diversity per service, 3) Improved fault isolation, 4) Better team autonomy, 5) Easier maintenance and updates, and 6) Enhanced resilience and reliability.",
...
router.get('/events', validateAuth, setupSSE);`,
  ...
  "description": "A production-ready Express component for secure drag-and-drop file uploads using Multer. Supports single and multiple uploads, file validation, metadata responses, and progress tracking.",
  "type": "storage",
  "version": "2.0.0",
  "tags": ["file", "upload", "multer", "express", "drag-drop", "storage", "nodejs", "javascript"],

  "documentation": {
    "overview": "A **drag-and-drop friendly file upload system** for Express using Multer. It supports both single and multiple file uploads with automatic file validation, storage, and clean JSON responses. Ideal for Next.js or React-based drag-drop frontends.",
    "installation": "npx backternity add file-upload-dragdrop",

    "commandDetails": {
      "purpose": "Installs a full-featured file upload system (single + multiple) compatible with drag-and-drop interfaces.",
      "creates": [
    ...
    "configuration": "# File Upload Configuration\nPORT=3001\nUPLOAD_DEST=uploads\nMAX_FILE_SIZE=5242880\nALLOWED_FILE_TYPES=image/jpeg,image/png,application/pdf\nSTORAGE_DRIVER=local",

    "frontendUsage": {
      "overview": "This backend pairs perfectly with drag-and-drop React components (like Dropzone or your own UI). Users can drop files which will be sent via `/api/upload/single` or `/api/upload/multiple`. The backend returns structured file metadata and URLs for previews.",
      "endpoints": [
        {
          "route": "/api/upload/single",
          "method": "POST",
        ...
        },
        {
          ...
          "method": "GET",
          "description": "Lists all uploaded files with metadata (for frontend preview grids).",
          "sampleResponse": {
            "success": true,
            "files": [
              {
    ...
    "usage": "// Example: Upload file programmatically\nconst formData = new FormData();\nformData.append('file', selectedFile);\nawait fetch('http://localhost:3001/api/upload/single', { method: 'POST', body: formData });",

    "advanced": {
      "overview": "Easily extendable to S3, MinIO, or GCP by swapping the Multer storage configuration. Use this structure for scalable drag-drop upload workflows.",
      "example": "// Example: S3 Integration\nconst multerS3 = require('multer-s3');\nconst AWS = require('aws-sdk');\nAWS.config.update({ accessKeyId: process.env.AWS_ACCESS_KEY_ID, secretAccessKey: process.env.AWS_SECRET_ACCESS_KEY });\nconst s3 = new AWS.S3();\nconst upload = multer({\n  storage: multerS3({\n    s3,\n    bucket: process.env.S3_BUCKET_NAME,\n    acl: 'public-read',\n    key: (req, file, cb) => cb(null, `uploads/${Date.now()}-${file.originalname}`)\n  })\n});"
    },
