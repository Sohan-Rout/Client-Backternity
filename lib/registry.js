...
app.get('/api/protected', validateAuth, (req, res) => {
  ...
  "postgresql-prisma-express-js": {
    ...
    documentation: {
      ...
      installation: "npx backternity add postgresql-prisma-express-js",
      ...
      frontendUsage: {
        ...
        example:
          "// app/components/PrismaUserManager.jsx\n'use client';\nimport { useState, useEffect } from 'react';\n\nexport default function PrismaUserManager() {\n  const [users, setUsers] = useState([]);\n  const [form, setForm] = useState({ name: '', email: '' });\n  const [loading, setLoading] = useState(false);\n  const [message, setMessage] = useState('');\n\n  async function fetchUsers() {\n    try {\n      const res = await fetch('/api/user');\n      const data = await res.json();\n      if (data.success) setUsers(data.users);\n    } catch (err) {\n      console.error('Failed to fetch users:', err);\n    }\n  }\n\n  useEffect(() => {\n    fetchUsers();\n  }, []);\n\n  async function handleSubmit(e) {\n    e.preventDefault();\n    setLoading(true);\n    setMessage('');\n\n    try {\n      const res = await fetch('/api/user', {\n        method: 'POST',\n        headers: { 'Content-Type': 'application/json' },\n        body: JSON.stringify(form)\n      });\n      const data = await res.json();\n\n      if (res.ok && data.success) {\n        setMessage('✅ User created successfully');\n        setForm({ name: '', email: '' });\n        fetchUsers();\n      } else {\n        setMessage('❌ ' + (data.error || 'Error creating user'));\n      }\n    } catch (error) {\n      setMessage('Error: ' + error.message);\n    } finally {\n      setLoading(false);\n    }\n  }\n\n  return (\n    <div className=\"max-w-md mx-auto bg-black/20 border border-white/10 rounded-xl p-6 text-white space-y-5\">\n      <h2 className=\"text-xl font-semibold\">PostgreSQL + Prisma User Manager</h2>\n\n      <form onSubmit={handleSubmit} className=\"space-y-3\">\n        <input\n          type=\"text\"\n          placeholder=\"Name\"\n          value={form.name}\n          onChange={(e) => setForm({ ...form, name: e.target.value })}\n          className=\"w-full px-3 py-2 rounded-md bg-white/10 border border-white/20 focus:outline-none\"\n          required\n        />\n        <input\n          type=\"email\"\n          placeholder=\"Email\"\n          value={form.email}\n          onChange={(e) => setForm({ ...form, email: e.target.value })}\n          className=\"w-full px-3 py-2 rounded-md bg-white/10 border border-white/20 focus:outline-none\"\n          required\n        />\n        <button\n          type=\"submit\"\n          disabled={loading}\n          className=\"w-full bg-emerald-500 hover:bg-emerald-600 text-white font-semibold py-2 rounded-md transition-colors\"\n        >\n          {loading ? 'Creating...' : 'Add User'}\n        </button>\n      </form>\n\n      {message && <p className=\"text-sm text-amber-300\">{message}</p>}\n\n      <div className=\"space-y-2\">\n        <h3 className=\"text-lg font-semibold border-b border-white/10 pb-1\">All Users</h3>\n        <ul className=\"divide-y divide-white/10\">\n          {users.length > 0 ? (\n            users.map((u) => (\n              <li key={u.id} className=\"py-2 flex justify-between\">\n                <span>{u.name}</span>\n                <span className=\"text-white/70 text-sm\">{u.email}</span>\n              </li>\n            ))\n          ) : (\n            <li className=\"text-sm text-white/50\">No users found.</li>\n          )}\n        </ul>\n      </div>\n    </div>\n  );\n}",
      },

      backendIntegration: {
        overview:
          "The backend exposes REST APIs for managing users in a PostgreSQL database using Prisma ORM. You can extend this structure to support multiple models and relationships easily.",
        example:
          "// server.js\nrequire('dotenv').config();\nconst express = require('express');\nconst cors = require('cors');\nconst bodyParser = require('body-parser');\nconst userRoute = require('./src/routes/user');\n\nconst app = express();\napp.use(cors());\napp.use(bodyParser.json());\napp.use('/api/user', userRoute);\n\napp.listen(process.env.PORT || 8000, () => {\n  console.log('🚀 Prisma PostgreSQL API running');\n});",
      },

      usage:
        "// Example server-side usage\nconst { getUsers, createUser, getUserById } = require('./src/services/userService');\n\n(async () => {\n  const newUser = await createUser({ name: 'John Doe', email: 'john@example.com' });\n  console.log('Created user:', newUser);\n\n  const users = await getUsers();\n  console.log('All users:', users);\n})();",

      securityTips: [
        "Never expose the database URL publicly or commit it to version control.",
        "Use environment variables for all credentials.",
        "Validate input data to prevent SQL injection (Prisma handles most cases, but always sanitize).",
        "Enable SSL for production PostgreSQL connections.",
        "Regularly run Prisma migrations to keep schema synchronized.",
      ],
    },
    ...
    meta: {
      ...
      scalable: true,
      category: "database",
      related: ["auth-jwt", "logger-winston", "rate-limiter"],
    },
  },
  "ecommerce-rest-api": {
    ...
    documentation: {
      overview: `A full-stack ready e-commerce backend built on Express and MongoDB.
Implements authentication, product catalog management, order processing, and Razorpay payment verification.
Follows clean MVC patterns with dedicated models, controllers, routes, and middleware. Designed for reuse across storefronts, admin dashboards, or mobile commerce apps.`,

      installation: "npx backternity add ecommerce-rest-api",
...
RATE_LIMIT_MAX=100`,
      ...
      frontendUsage: {
        ...
        endpoints: [
          ...
          },
          {
            ...
            sampleResponse: {
              ...
              total: 120,
              data: [
                { id: "prod_1", name: "Laptop", price: 69999 },
                { id: "prod_2", name: "Shoes", price: 2999 },
              ],
            },
          },
          {
            ...
            contentType: "application/json",
            sampleRequest: {
              orderItems: [{ product: "prod_1", qty: 2, price: 499 }],
              shippingAddress: {
                address: "123 Street",
                city: "Mumbai",
                postalCode: "400001",
          ...
          {
            route: "/api/v1/orders/:id/payment/verify",
            method: "POST",
            description:
        ...
        example: `// Example React flow for Razorpay Checkout
import { useState } from 'react';
...
export default function CheckoutButton() {
  ...
  async function createOrder() {
    ...
    const rzp = new window.Razorpay({
      ...
      handler: async function (paymentResponse) {
        ...
        await fetch('/api/v1/orders/' + data.orderId + '/payment/verify', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json',
                     Authorization: 'Bearer ' + localStorage.getItem('token') },
  ...
  return (
    <button onClick={createOrder} disabled={loading}>
      ...
      backendIntegration: {
        overview:
          "Import the models, mount routes under the configured API prefix, ensure MongoDB is running, and supply correct Razorpay credentials.",
        example: `// server.js
const express = require('express');
const dotenv = require('dotenv');
dotenv.config();
...
app.use(process.env.API_PREFIX, productRoutes);
app.use(process.env.API_PREFIX, orderRoutes);
...
app.listen(process.env.PORT || 3001, () => {
  console.log('E-commerce API running...');
      ...
      extendability: {
        overview:
          "The architecture allows replacement or extension of models, payment providers, validation layers, and business rules.",
        example: `// Example: Adding new product fields
const mongoose = require('mongoose');
...
module.exports =
  mongoose.models.ExtendedProduct ||
  Product.discriminator('ExtendedProduct', ExtendedProductSchema);`,
      },
...
router.post('/products', protect, admin, (req, res) => {
  res.json({ message: 'Admin access granted' });
});`,

      securityTips: [
        "Use a long, unpredictable JWT secret.",
        "Store Razorpay keys securely and rotate when required.",
        "Enable HTTPS in production for all payment flows.",
        "Apply stricter rate limiting to login and payment endpoints.",
        "Validate all product and order payloads before processing.",
      ],
    },
  },
  "auth-multi-factor": {
...
app.get('/api/protected', validateAuth, (req, res) => {
  ...
  "auth-oauth-google": {
    ...
    documentation: {
      overview: `A developer-ready **Google OAuth 2.0** authentication component built with **Passport.js** and **JWT**.
...
app.use(rateLimiter);`,
  ...
  "logger-winston": {
    ...
    documentation: {
      overview: `A robust, developer-ready logging component built on **Winston**, providing structured, leveled, and environment-based logging for Express and Node.js apps.
...
console.log(status);`,
  ...
  "user-presence-tracker-js": {
    ...
    documentation: {
      overview: `A **plug-and-play** real-time user presence system for Express apps. Tracks **online/offline status**, **activity timestamps**, and **heartbeats** using WebSocket + Redis. 
...
**Call one endpoint from ANY frontend event** ('POST /api/presence/activity') to register user activity. Auto-handles WebSocket connect/disconnect for instant status updates. Perfect for chat apps, dashboards, and social platforms.`,
      ...
      installation: "npx backternity add user-presence-tracker-js",
      commandDetails: {
        purpose:
      ...
      configuration: `# Redis Configuration (required)
REDIS_HOST=127.0.0.1
REDIS_PORT=6379
...
PORT=3001`,
      ...
      frontendUsage: {
        ...
        endpoints: [
          ...
          {
            route: "/api/presence/status/:userId",
            method: "GET",
            description: "Get single user online status + last activity",
            sampleRequest: null,
            sampleResponse: {
              success: true,
              data: {
          ...
          {
            route: "/api/presence/online/count",
            method: "GET",
            description: "Get total online users count",
            sampleRequest: null,
            sampleResponse: {
              success: true,
              data: { count: 47 },
          ...
          {
            route: "/api/presence/online/users",
            method: "GET",
            description: "Get list of all online user IDs",
            sampleRequest: null,
            sampleResponse: {
              success: true,
              data: {
        ...
        example: `// app/components/PresenceTracker.jsx (React/Next.js)
'use client';
import { useState, useEffect } from 'react';
import io from 'socket.io-client';

export default function PresenceTracker({ userId }) {
  const [status, setStatus] = useState('');
  ...
  useEffect(() => {
    ...
    socket.on('status', (data) => setStatus('🟢 Online'));
    socket.on('user_offline', (data) => console.log('User offline:', data.userId));
    socket.on('user_activity', (data) => console.log('User active:', data.userId));

    return () => socket?.disconnect();
  }, [userId]);

  // Activity ping on ANY event
  const trackActivity = async () => {
    try {
      ...
      if (res.ok) setStatus('✅ Activity tracked');
    } catch {
      setStatus('⚠️ Activity tracking failed');
    }
  };
  ...
  return (
    <div className="p-6 border border-white/10 rounded-xl bg-gradient-to-r from-black/50 to-gray-900">
      ...
      <div className="space-y-2 text-sm">
        ...
        <p><strong>Online Users:</strong> {onlineCount}</p>
        <p className="text-xs text-gray-400">
          💡 Click ANYWHERE on page → auto-tracks activity via REST API<br/>
          🔌 WebSocket auto-handles online/offline
        </p>
      </div>
    </div>
  );
      ...
      backendIntegration: {
        overview:
          "Add **3 lines** to your existing Express server. Works with any auth system.",
        example: `// server.js - ADD THESE 3 LINES
const express = require('express');
...
// 🚀 PRESENCE TRACKER INTEGRATION (3 LINES)
app.use('/api/presence', presenceRoutes);
const { setSocketIO } = require('./src/routes/presence');
setSocketIO(io);  // Handles all WebSocket logic

server.listen(3001, () => {
  console.log('✅ Presence Tracker ready!');
});`,
      },

      usage: `// From ANY frontend event (click, scroll, mouse move, etc.)
...
// Check user status
fetch('/api/presence/status/user_123').then(r => r.json());

// WebSocket (auto online/offline)
const socket = io('/', { query: { userId: 'user_123' } });
socket.on('user_offline', handleUserOffline);`,

      securityTips: [
        "Validate `userId` server-side to prevent spoofing",
        "Add auth middleware to presence routes for production",
        "Use Redis ACLs and TLS for production deployments",
        "Set short TTLs (5-15min) for inactive users",
        "Monitor Redis memory usage for large user bases (>10k concurrent)",
        "Consider Socket.IO auth for WebSocket connections",
      ],
    },
    ...
    meta: {
      ...
      scalable: true,
      category: "real-time",
      related: ["job-queue-bull", "auth-jwt", "websocket-chat"],
    },
  },
...
await createNotification({
  ...
  "ai-openai-chat": {
    ...
    documentation: {
      overview: `A lightweight and developer-ready AI chat component built on OpenAI’s GPT models.
...
router.get('/events', validateAuth, setupSSE);`,
  ...
  "socket-io": {
    ...
    documentation: {
      ...
      installation: "npx backternity add socket-io",
      ...
      frontendUsage: {
        ...
        endpoints: [
          {
            ...
            sampleResponse: {
              success: true,
              message: "Event broadcast globally",
              data: {
                event: "notification",
                timestamp: "2024-01-20T14:25:00Z",
              },
          ...
          {
            route: "/api/socket/send-room",
            method: "POST",
            description: "Broadcasts an event to a specific Socket.IO room.",
            sampleRequest: {
              room: "admins",
              event: "admin_alert",
              data: {
                message: "Critical CPU threshold reached",
              },
            },
            sampleResponse: {
              success: true,
              message: "Event sent to room admins",
              data: {
                room: "admins",
                event: "admin_alert",
                timestamp: "2024-01-20T14:25:00Z",
        ...
        example:
          "// Example: React / Next.js frontend integration\nimport { useEffect, useState } from 'react';\nimport { io } from 'socket.io-client';\n\nexport default function RealtimeSocket() {\n  const [messages, setMessages] = useState([]);\n  const [connected, setConnected] = useState(false);\n\n  useEffect(() => {\n    const socket = io('http://localhost:3001');\n\n    socket.on('connected', (info) => {\n      setConnected(true);\n      console.log('Connected:', info);\n    });\n\n    socket.on('notification', (data) => {\n      setMessages((prev) => [...prev, data]);\n    });\n\n    socket.emit('join_room', 'dashboard');\n    return () => socket.disconnect();\n  }, []);\n\n  return (\n    <div className=\"max-w-md mx-auto p-4\">\n      <h2 className=\"text-lg font-semibold mb-3\">Live Socket.IO Updates</h2>\n      <p>Status: {connected ? 'Connected' : 'Connecting...'}</p>\n      <ul className=\"space-y-2 mt-3\">\n        {messages.map((msg, i) => (\n          <li key={i} className=\"border rounded-md p-2 bg-secondary/40\">\n            {msg.message || JSON.stringify(msg)}\n          </li>\n        ))}\n      </ul>\n    </div>\n  );\n}",
      },

      backendIntegration: {
        overview:
          "Mount Socket.IO on the HTTP server and expose REST endpoints for event broadcasting.",
        example:
      ...
      usage:
        "// Send global event programmatically\nawait fetch('http://localhost:3001/api/socket/send', {\n  method: 'POST',\n  headers: { 'Content-Type': 'application/json' },\n  body: JSON.stringify({ event: 'notification', data: { message: 'Order shipped' } })\n});\n\n// Send event to a specific room\nawait fetch('http://localhost:3001/api/socket/send-room', {\n  method: 'POST',\n  headers: { 'Content-Type': 'application/json' },\n  body: JSON.stringify({ room: 'admins', event: 'alert', data: { message: 'CPU over 90%' } })\n});",

      advanced: {
        overview:
          "Supports Redis scaling, authentication middleware, and presence tracking for distributed real-time deployments.",
        example:
          "// Example: Redis Scaling Adapter\nconst { createAdapter } = require('@socket.io/redis-adapter');\nconst Redis = require('ioredis');\nconst pub = new Redis();\nconst sub = new Redis();\nio.adapter(createAdapter(pub, sub));\n\n// Auth-protected room join\nsocket.on('join_room', async ({ room, token }) => {\n  if (await validateToken(token)) socket.join(room);\n  else socket.emit('error', { message: 'Unauthorized' });\n});",
      },

      securityTips: [
        "Use HTTPS to protect WebSocket traffic.",
        "Authenticate room joins for private channels.",
        "Validate event payloads from clients.",
        "Rate-limit REST broadcasting routes.",
