  ...
  return (
    <section className="py-24 relative overflow-hidden">
      ...
      <motion.div
        ...
        className="absolute left-1/3 top-1/4 w-[28rem] h-[28rem] bg-primary/10 rounded-full blur-[120px]"
      />
        ...
        >
          {componentCategories.map((category, index) => {
            ...
            return (
              <motion.div
                key={index}
                className="group relative p-6 rounded-xl border border-white/[0.05] bg-gradient-to-br from-background/50 to-background/40 backdrop-blur-sm hover:shadow-[0_8px_32px_rgba(0,0,0,0.25)] hover:border-primary/20 transition-all duration-300"
                whileHover={{ y: -4 }}
              >
                {/* Soft highlight ring */}
                ...
                <div className="flex items-center mb-4 relative z-10">
                  <div className="w-11 h-11 rounded-lg bg-primary/10 flex items-center justify-center mr-4 group-hover:bg-primary/15 transition-colors">
                ...
                <ul className="space-y-2 relative z-10">
                  {category.examples.map((example, idx) => (
                    ...
                    >
                      <div className="w-1.5 h-1.5 bg-primary rounded-full mr-3 opacity-70 group-hover:opacity-100 transition-opacity" />
                      {example}
                    </li>
                  ))}
                </ul>
